...
function getStockSignal(product: Product) {
  ...
  if (available <= 3) {
    return {
      ...
      className: "bg-[#fff1e8] text-[#8a3f22]",
    };
  }
  ...
  if (available <= 10) {
    return {
      label: "Low Stock",
      className: "bg-[#fff4dc] text-[#7a6426]",
    };
  }

  return {
    label: "In Stock",
    className: "bg-[#f0eadf] text-[#5f4a28]",
  };
}
  ...
  function renderFilterControl(filter: CategoryFilter, compact = false) {
    return (
      <div key={filter.key} className={compact ? "border-b border-stone-200 py-4" : "relative"}>
        {compact ? (
          <p className="text-sm font-medium text-[#241f18]">{filter.label}</p>
        ) : (
          <p className="mb-2 text-xs font-medium uppercase tracking-[0.12em] text-[#7a6426]">
            {filter.label}
          </p>
        )}
        <div className={compact ? "mt-3 grid grid-cols-2 gap-2" : "flex min-w-44 flex-col gap-1 rounded-lg border border-[#e6d6b9] bg-white p-2 shadow-sm"}>
          {filter.options.map((option) => (
            <button
              ...
              className={`rounded-md px-3 py-2 text-left text-xs font-medium transition ${
                filterDraft[filter.key] === option.value
                  ? "bg-[#241f18] text-[#fffaf1]"
      ...
      <section className="border-b border-[#e5d6bd] bg-white">
        <div className="mx-auto max-w-7xl px-4 py-5">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              <h2 className="text-lg font-medium text-[#241f18]">Select Your Category</h2>
              <p className="text-sm text-stone-500">Jump to related category shelves.</p>
            </div>
            <Link
              href={viewAllHref}
              className="w-fit rounded-full border border-[#d5b46b] px-4 py-2 text-xs font-medium uppercase text-[#5f4a28]"
            >
          ...
          {listingShortcuts.length > 0 && (
            <div className="mt-4 flex gap-2 overflow-x-auto pb-1">
              ...
              <Link
                key={shortcut.slug}
                href={shortcut.href}
                className="shrink-0 rounded-full border border-[#e2cfaa] bg-[#fbf7ef] px-4 py-2 text-sm font-medium text-[#4f463b] hover:border-[#b58b3b] hover:bg-white"
              >
                {shortcut.label}
              </Link>
              ))}
        ...
        <div className="mb-4 rounded-md border border-[#e4d5bc] bg-white p-3 shadow-sm">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            ...
            <button
              ...
              className="rounded-full bg-[#241f18] px-4 py-2 text-sm font-medium text-[#fffaf1] lg:hidden"
            >
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2">
            {selectedChips.map((chip) => (
              <button
                ...
                className="rounded-full bg-[#241f18] px-3 py-1.5 text-xs font-medium text-[#fffaf1]"
              >
                {chip.label} x
              </button>
            ))}
            {selectedChips.length > 0 && (
              <button
                ...
                className="rounded-full border border-[#d5b46b] px-3 py-1.5 text-xs font-medium text-[#5f4a28]"
              >
                Clear All
              </button>
            )}
            <button
              ...
              className="hidden rounded-full bg-[#d5b46b] px-4 py-1.5 text-xs font-medium text-[#241f18] lg:inline-flex"
            >
              Apply
            </button>
          </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/40 lg:hidden">
          <div className="absolute inset-x-0 bottom-0 max-h-[88vh] overflow-y-auto rounded-t-2xl bg-white p-4 shadow-2xl">
            <div className="sticky top-0 z-10 flex items-center justify-between gap-3 border-b border-stone-200 bg-white pb-3">
              <div>
                <h2 className="text-lg font-medium text-[#241f18]">Filter & Sort</h2>
            ...
            <div className="sticky bottom-0 mt-4 grid grid-cols-2 gap-3 border-t border-stone-200 bg-white pt-3">
              <button
                type="button"
                onClick={clearFilters}
                className="rounded-full border border-[#241f18] px-4 py-3 text-sm font-medium text-[#241f18]"
              >
                Clear All
              </button>
              <button
                type="button"
                onClick={() => applyFilters()}
                className="rounded-full bg-[#241f18] px-4 py-3 text-sm font-medium text-[#fffaf1]"
              >
                Apply
              </button>
            </div>
